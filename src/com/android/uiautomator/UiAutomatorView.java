...
public class UiAutomatorView extends Composite {
    ...
    public UiAutomatorView(Composite parent, int style) {
        ...
        mScreenshotCanvas.addControlListener(new ControlListener() {
            @Override
    ...
    public void loadAttributeTable() {
        ...
        Object[] attributesArray = mModel.getSelectedNode().getAttributesArray();
        //新增
        List<AttributePair> addPair = new ArrayList<>();
        for (Object attribute : attributesArray) {
            if (attribute instanceof AttributePair) {
                AttributePair attributePair = (AttributePair) attribute;
                String key = attributePair.key;
                String val = attributePair.value;
                if (key != null && key.length() != 0) {
                    if (key.equals("bounds")) {
                        String[] sp = val.split("]");
                        int left = Integer.parseInt(val.substring(1, val.indexOf(",")));
                        int top = Integer.parseInt(val.substring(val.indexOf(",") + 1, val.indexOf("]")));
                        int right = Integer.parseInt(val.substring(val.lastIndexOf("[") + 1, val.lastIndexOf(",")));
                        int bottom = Integer.parseInt(val.substring(val.lastIndexOf(",") + 1, val.lastIndexOf("]")));
                        java.awt.Rectangle rectangle = new java.awt.Rectangle(left, top, right - left, bottom - top);
                        String center = (int)rectangle.getCenterX() + "," + (int)rectangle.getCenterY()+"";
                        addPair.add(new AttributePair("中心点", center));
                    }
                }
            }
        }
        if (addPair.size() != 0) {
            int oldSize = attributesArray.length;
            attributesArray = Arrays.copyOf(attributesArray, attributesArray.length + addPair.size());
            for (int i = oldSize, j = 0; i < attributesArray.length; i++, j++) {
                attributesArray[i] = addPair.get(j);
            }
        }
        mTableViewer.setInput(attributesArray);
    }
